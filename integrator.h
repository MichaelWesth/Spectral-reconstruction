...
class clusterAlg{
    ...
    void calcAve(std::vector<std::vector<bool>>& cluster, std::vector<std::vector<std::complex<double>>>& avg, const int& i){
        avg = std::vector<std::vector<std::complex<double>>>(T,std::vector<std::complex<double>>(d,0));
        
        for (int i = 0; i < T; i++) 
            for (int j = 0; j < L; j++)
                if (cluster[j][i]){
                    double a = p0*(double)(j-i);
    ...
    public:
        ...
        void update(const int& cluster_count){
            cluster_r = std::vector<std::vector<bool>>(L, std::vector<bool>(T,false));
            unit_vec(r);
            getCluster(cluster_r, r, cluster_size_r, i_r);
            calcAve(cluster_r, avg_r, i_r);
            if (cluster_count == 2){
                cluster_u = std::vector<std::vector<bool>>(L, std::vector<bool>(T,false));
                unit_vec(u);
                orthogonal_vec(r, u);
                getCluster(cluster_u, u, cluster_size_u, i_u);
                calcAve(cluster_u, avg_u, i_u);
                calcAve_all(avg_all);
        ...
        void getCluster(std::vector<std::vector<bool>>& cluster, std::vector<double>& reflec, int& size, int& first) {
        ...
        std::vector<double> three_point_obs(const int& t3, const int& t2, const int &t1) {
            std::vector<std::complex<double>> tmp(2,0);
            for (int j = 0; j < T; j++) {
                tmp[0] += dot_complex(avg_r[(j+t1+T)%T],r) * dot_complex(avg_r[(j+t2+1+T)%T],r) * dot_complex(avg_u[(j+t2+T)%T],u) * dot_complex(avg_u[(j+t3+T)%T],u);
                tmp[1] += dot_complex(avg_r[(j+t1+T)%T],r) * dot_complex(avg_u[(j+t2+1+T)%T],u) * dot_complex(avg_r[(j+t2+T)%T],r) * dot_complex(avg_u[(j+t3+T)%T],u);
            }
            tmp[0] *= sqrt((double)L)*(double)T / (double)(cluster_size_r*cluster_size_u);
            tmp[1] *= sqrt((double)L)*(double)T / (double)(cluster_size_r*cluster_size_u);
            return {tmp[0].real(), tmp[1].real()};  
        }
